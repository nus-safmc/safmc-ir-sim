    ...
    def __post_init__(self) -> None:
        ...
        for lm in self.landmarks:
            ...
            if isinstance(lm, Target):
                raise ConfigError(
                    f"landmark {lm.id!r} is a Target. Targets are generated by the arena and "
                    f"scored by the mission; one placed here would be seen by the camera as a "
                    f"{lm.kind!r} but could never score. Use a plain Landmark with its own kind."
                )
            if lm.id in seen:
...
def generate_arena(
    ...
    # A placed body -- an anchor on a post, a prop -- is a fixed feature of the venue, so the
    # random layout is generated around it: walls and pillars keep their published gaps from
    # it, targets are not dropped onto it, and the room is re-drawn if a wall would cut it.
    fixed = [target_polygon(lm) for lm in cfg.landmarks if lm.solid]
    structure.extend(fixed)
    ...
    for attempt in range(cfg.max_placement_attempts):
        ...
        if not any(w.polygon().intersects(body) for w in room for body in fixed):
            break
    else:
        raise ArenaError(
...
def validate_arena(spec: ArenaSpec, drone_radius_m: float = DRONE_RADIUS_M) -> None:
    ...
    1. Every solid landmark -- every target, every placed body -- is outside every obstacle
       and clear of every other solid landmark.
    ...
    4. Every landmark is inside the field and has a unique id (R-WORLD-7).
    """
    _validate_landmarks(spec)
    _validate_solid_landmarks_clear(spec)
...
def _validate_solid_landmarks_clear(spec: ArenaSpec) -> None:
    solid = [lm for lm in spec.all_landmarks if lm.solid]
    polys = [target_polygon(lm) for lm in solid]
    obstacles = spec.obstacle_polygons()
    if obstacles:
        tree = STRtree(obstacles)
        for lm, poly in zip(solid, polys):
            for j in tree.query(poly):
                if obstacles[j].intersects(poly):
                    raise ArenaError(
                        f"landmark {lm.id} at ({lm.x:.2f}, {lm.y:.2f}) overlaps an obstacle"
                    )
    for i, (a, pa) in enumerate(zip(solid, polys)):
        for b, pb in zip(solid[i + 1:], polys[i + 1:]):
            if pa.intersects(pb):
