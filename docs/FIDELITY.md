...
| A-14 | DW3000 line-of-sight ranging noise | 0.05 m std | Range to one anchor over a tape measure, 1-20 m; the residual's std is this number and its slope is the calibration (F-26). Bracketed by the DW3000 datasheet's 1.5 cm (Table 14, calibrated, at -85 dBm) and two independent measurements of the part at 5.7-6 cm mean/median error. The default is ~30% optimistic against those (F-30) |
| A-15 | DW3000 maximum range | 20 m | **Cheap and decisive, and it is a firmware setting more than a chip limit.** Walk a tag away from an anchor in the hall until ranging stops -- *in the configuration you will fly*. 20 m is a stock 6.8 Mb/s board in an office; Qorvo call 40-50 m typical for line of sight; 850 kb/s with a 4096-symbol preamble has been measured past 90 m indoors. At 20 m three Start Area anchors reach every point of the field; at 12 m the far third hears none (F-29) |
| A-16 | DW3000 through-wall bias and spread | +0.15 m, 0.40 m std | Range through one of the venue's walls, and a pillar. **No DW3000 study publishes a through-wall bias in the form this model needs**: the aggregate is a mean absolute error of 46.7 cm in an office (Ember et al., IFIP 2024), and a second source reporting per-obstacle medians of 0.08-0.57 m is unconfirmed here. The spread is still the DW1000's: 1.39 ns = 0.42 m, rounded (Kolakowski & Modelski, TELFOR 2017, Table 1, read in full). See F-28 |
| A-17 | DW3000 through-wall dropout probability | 0.10 | The same experiment: the fraction of sweeps that return nothing. No published rate for either part. Note a tag configured for *secure* ranging drops far more -- one study saw 802.15.4z timestamping fail 10-40% of the time with packet reception at 100%, and over half under Wi-Fi 6E interference |
| A-18 | DW3000 outlier probability and size | 0 (off), up to 1.5 m | A long line-of-sight log with people walking through; count errors beyond three sigma. The tail is documented, its rate is not, so it is off until measured -- and it is the term that would close this model's largest gap to the measured part, whose non-line-of-sight 90th percentile is 129.5 cm against this model's 70 cm (F-30) |
...
**A-15 is the one to measure first for anything built on the UWB tag.** Every result on
`sensors/uwb.py` is conditional on five numbers nobody has measured on the team's own kit;
reach is the one that changes the answer to "where do the anchors go". The part is now
chosen -- the Qorvo DW3000 -- so these are no longer guesses about an unknown radio, but
none of them has been measured *on a DW3000 in this hall*, which is what would settle them.
...
| F-22 | — | `examples/03_custom_sensor.py` ships a range-only beacon sensor | A **template for the sensor contract, not a model of any hardware**: uncalibrated, invented noise, no wall bias, and the airframe carries no UWB. Any sensor added on the contract owes this ledger an entry before its readings are quoted. *Since ADR-0006 the model is `sensors/uwb.py` (F-23..F-32 below); the template stays a template* |
| F-23 | A DW3000 tag ranges to its anchors one at a time inside its own TDMA slot: a double-sided exchange is three frames of ~170 us, so ~0.51 ms per anchor, and a shipping firmware's slot is 10 ms at 6.8 Mb/s | Every anchor measured in the same tick | Ten anchors take ~5.1 ms of that slot, so the first-to-last skew is ~0.2 cm at cruise speed -- comfortably below the LOS noise. What is not modelled is the up-to-one-frame latency between a sweep and the tick that reports it. The same simplification A-8 makes for the ring |
| F-24 | Through-wall error depends on the material and on how many walls are crossed: the source of A-16 measured four times the bias behind several walls (+0.58 m, 0.61 m spread), through pre-stressed concrete panels that still ranged; metal is where a link dies | One boolean -- obstructed or not -- with the one-wall pair (A-11) behind any number of walls. Markers, placed bodies and teammates are transparent | Optimistic behind two or more walls; the multi-wall pair is the number to start from, and counting crossings is one raycast helper away. The venue's material is unmeasured |
...
| F-28 | The through-wall **spread** is a DW1000 number in a DW3000 model | A-16's 0.40 m comes from Kolakowski & Modelski's DW1000 measurement, because no DW3000 study publishes a per-obstacle standard deviation | The bias and the spread come from different radios and different buildings. The DW1000 evidence was read in full here; the DW3000 evidence was not, and two independent searches disagreed on whether a DW3000 through-wall study exists at all. This is the weakest joint in the model |
| F-29 | DW3000 range varies 3-5x with data rate, preamble length and PAC -- 850 kb/s beats 6.8 Mb/s by about 6 dB, and PAC 8 to 16 alone bought 2-3x in one study -- and the part has two channels, of which 9 is ~1 dB worse and draws ~1.8x the idle current | One scalar `max_range_m`, no channel, no data rate, no preamble | The configuration is worth more than the hardware here, and the model cannot express it. A run quoting coverage should say which radio configuration A-15 was measured in |
| F-30 | Measured DW3000 error: mean absolute 5.7 cm in line of sight and 46.7 cm behind an obstruction, 90th percentiles 13.7 cm and 129.5 cm (Ember et al., IFIP 2024, office, channel 9) | This model, at its defaults, produces 4.0 cm / 34.1 cm and 8.2 cm / 70.3 cm | **The model is optimistic against the measured part in all four statistics, and worst in the tail.** The body is within a factor of two; the 90th percentile is out by 1.8x because the tail is Gaussian and the real one is not. `outlier_probability` (A-18) is the term that closes it, and it is off. `los_noise_std_m=0.07` matches the measured line-of-sight *mean absolute error*; its 90th percentile stays ~16% optimistic, because no Gaussian matches both. A test pins this comparison so it cannot drift |
| F-32 | The sweep rate is `1 / (n_tags * slot)`: TDMA slots belong to tags, so a bigger swarm ranges less often on the same radio -- 10 Hz at ten drones, 4 Hz at twenty-five | A fixed `rate_hz` that knows nothing about the fleet | A run that changes `n_drones` without changing `rate_hz` silently ranges faster than the hardware would, and the error grows with the fleet: at 25 drones the default 10 Hz is 2.5x optimistic. `uwb.sweep_rate_hz()` computes the right value and the example uses it, but nothing enforces it -- a sensor config cannot see `RunConfig.n_drones`, and wiring the two together would make a sensor's timing depend on the fleet invisibly |
| F-31 | Antenna-delay calibration is a per-unit constant offset: +/-15 cm uncalibrated against +/-6 cm calibrated (datasheet Table 14), and a DWM3000 module ships with none while a DWM3001C ships factory-calibrated | No per-anchor constant bias at all | Which module the team buys changes the error by 15 cm in a way this model cannot represent, because its only bias is the through-wall one. One independent study measured 16.03 cm RMSE on an uncalibrated DW3000 *and reported seeing no clear improvement from calibrating it*, which is a warning about the procedure, not a licence to skip it. Extends F-26 |
