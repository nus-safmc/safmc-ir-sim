...
| F-19 | `TOF_SENSOR_FOV_RAD` (45 deg) and `TOF_RATE_HZ` (15 Hz) are recorded but not used | The ring has no beam width and samples synchronously at the tick rate | Both are sensor physics we wrote down and did not implement. F-1 and F-18 are their consequences |
