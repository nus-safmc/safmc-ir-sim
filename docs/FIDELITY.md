...
| F-28 | The through-wall **spread** is a DW1000 number in a DW3000 model | A-11's 0.40 m comes from Kolakowski & Modelski's DW1000 measurement, because no DW3000 study publishes a per-obstacle standard deviation | The bias and the spread come from different radios and different buildings. The DW1000 evidence was read in full here; the DW3000 evidence was not, and two independent searches disagreed on whether a DW3000 through-wall study exists at all. This is the weakest joint in the model |
| F-29 | DW3000 range varies 3-5x with data rate, preamble length and PAC -- 850 kb/s beats 6.8 Mb/s by about 6 dB, and PAC 8 to 16 alone bought 2-3x in one study -- and the part has two channels, of which 9 is ~1 dB worse and draws ~1.8x the idle current | One scalar `max_range_m`, no channel, no data rate, no preamble | The configuration is worth more than the hardware here, and the model cannot express it. A run quoting coverage should say which radio configuration A-10 was measured in |
| F-30 | Measured DW3000 error: mean absolute 5.7 cm in line of sight and 46.7 cm behind an obstruction, 90th percentiles 13.7 cm and 129.5 cm (Ember et al., IFIP 2024, office, channel 9) | This model, at its defaults, produces 4.0 cm / 34.2 cm and 8.2 cm / 70.3 cm | **The model is optimistic against the measured part in all four statistics, and worst in the tail.** The body is within a factor of two; the 90th percentile is out by 1.8x because the tail is Gaussian and the real one is not. `outlier_probability` (A-13) is the term that closes it, and it is off. `los_noise_std_m=0.07` matches the measured line-of-sight error. A test pins this comparison so it cannot drift |
